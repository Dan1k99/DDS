...
input_DDS_button = connector.getInput("Actuator1::Actuator_button")
input_DDS_temp1 = connector.getInput("Actuator1::Actuator_Tempsensor1")

# Configure all Outputs (Publishers of Actuator)
output_DDS_actuator_status = connector.getOutput("actuator_publisher1::status_writer")
output_DDS_temp_extreme = connector.getOutput("actuator_publisher2::temp_extreme_writer")
...
while True:
    ...
    if status_index != 2:  # If actuator is working, connect to all sensors
        input_DDS_temp1.take()
        num_samples_temp1 = input_DDS_temp1.samples.getLength()

        # Receive all temperature information
        for i in range(num_samples_temp1):
            if input_DDS_temp1.infos.isValid(i):
                temp1_message = input_DDS_temp1.samples.getNumber(i, "Temp")

        temp1_status = get_temp_status(num_samples_temp1)  # Check which temps are running

        # Checking for extreme temperatures
        if temp1_status:
            if temp1_message < 18 or temp1_message > 45:
                output_DDS_temp_extreme.instance.setNumber("Tempsensor1_extreme", temp1_message)
    ...
    output_DDS_actuator_status.instance.setString("ActuatorStatus", actuator_status_list[status_index])
